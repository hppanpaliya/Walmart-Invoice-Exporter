...
import { loadSandbox, evalIn } from './helpers/sandbox';
...
test('displayOrdersFromDb: reads ONLY the active provider\'s OrderDb partition', async () => {
  ...
  await OrderDb.putSummaries({ a1: { orderDate: '2026-02-01T00:00:00.000Z' } }, {}, 'WALMART_CA');
  sandbox.window.Sidepanel.state.app.provider = 'WALMART_CA';
  const calls = spyOnDisplayOrderNumbers(sandbox);

  const shown = await sandbox.window.Sidepanel.actions.displayOrdersFromDb(null);

  assert.equal(shown, true);
  assert.deepEqual(calls[0].orderNumbers, ['a1'], 'Walmart records must not leak into the Walmart.ca view');
});

test('displayOrdersFromDb: the combined "All providers" view unions every enabled provider\'s records', async () => {
  const sandbox = loadProviderAwareSandbox();
  // Opt Walmart.ca in (flags live under settings.flags) so scopeIds(ALL) includes it.
  await new Promise((resolve) =>
    sandbox.chrome.storage.local.set({ settings: { flags: { 'provider.walmart_ca': true } } }, resolve)
  );
  const OrderDb = evalIn(sandbox, 'OrderDb');
  await OrderDb.putSummaries({ w1: { orderDate: '2026-01-01T00:00:00.000Z' } }, {}, 'WALMART_US');
  await OrderDb.putSummaries({ a1: { orderDate: '2026-02-01T00:00:00.000Z' } }, {}, 'WALMART_CA');
...
test('displayOrdersFromDb: another provider\'s in-flight collection never overlays into this view', async () => {
  ...
  assert.deepEqual(calls[0].orderNumbers, ['a1'], 'the Walmart overlay must not bleed into the Walmart.ca view');
});

test('renderOrderList: a foreign provider\'s raw progress numbers are not rendered as a fallback either', async () => {
  const sandbox = loadProviderAwareSandbox();
  const app = sandbox.window.Sidepanel.state.app;
  app.provider = 'WALMART_CA'; // empty Walmart.ca partition
  ...
  // The foreign crawl's numbers are NOT rendered; the Walmart.ca view is cleared
  // to empty (one call, no orders) rather than showing 'w9'.
  assert.equal(calls.length, 1, 'an empty Walmart.ca view stays empty while a Walmart crawl runs');
  assert.deepEqual(calls[0].orderNumbers, []);
});
...
test('handleStartCollection: a non-Walmart provider collects via its own ordersListUrl from ANY tab', async () => {
  ...
  const caUrl = evalIn(sandbox, "ProviderRegistry.getById('WALMART_CA').ordersListUrl");
  assert.equal(sent.length, 1);
  assert.equal(sent[0].url, caUrl, 'the background worker opens the adapter\'s own orders list');
  assert.equal(sent[0].provider, 'WALMART_CA');
  assert.equal(app.collectionProvider, 'WALMART_CA');
});
