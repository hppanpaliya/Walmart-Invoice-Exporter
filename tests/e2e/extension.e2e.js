...
test('extension end-to-end', async (t) => {
  ...
  try {
    ...
    // The in-panel dashboard view is gone — the spending dashboard is now a
    // full extension page (dashboard.html) with its own coverage. The panel
    // still supports pre-set filters/selections via applyListFilter, which
    // the dashboard's tap-throughs drive through the embed bridge.
    await t.test('applyListFilter selects exactly the requested orders in the main list', async () => {
      await panel.evaluate((orderNumber) => {
        window.Sidepanel.view.applyListFilter({ filter: 'all', selectOrders: [orderNumber] });
      }, INSTORE_ORDER);

      assert.equal(
        await panel.isChecked(`input[value="${INSTORE_ORDER}"]`), true,
        'the requested order must be selected'
      );
      assert.equal(
        await panel.isChecked(`input[value="${ONLINE_ORDER}"]`), false,
        'orders outside the requested selection must NOT be selected'
      );
