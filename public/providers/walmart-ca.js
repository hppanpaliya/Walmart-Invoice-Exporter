/**
 * providers/walmart-ca.js — Walmart.ca (WALMART_CA) provider adapter.
 *
 * A CONFIG VARIANT of the Walmart.com adapter (providers/walmart-us.js).
 * Walmart.ca runs the SAME Orchestra/Glass platform as .com: same __NEXT_DATA__
 * shape, same GraphQL PurchaseHistoryV3 payload, same print-view DOM selectors.
 * Once the user is logged in (via the identity.walmart.com OAuth flow) the
 * session cookie carries and same-origin in-page requests just work — NO token
 * capture needed (live-verified 2026-07-18, see docs/recon-findings.md).
 *
 * Deltas vs Walmart.com (the ONLY site-specific differences):
 *  - host                www.walmart.ca (vs www.walmart.com)
 *  - orders list URL     https://www.walmart.ca/orders
 *  - API path segment    /orchestra/graphql/PurchaseHistoryV3/<hash>
 *                        (vs .com's /orchestra/cph/graphql/…) — the page's own
 *                        client issues this; we capture it passively (below).
 *  - tenant              CA_GLASS  (carried by the page's own request)
 *  - pagination          cursor-based (variables.input.cursor) — driven by the
 *                        page's own "next" button, same as .com's UI.
 *  - product link base   https://www.walmart.ca/ip/<usItemId>
 *  - locale/currency     en-CA / CAD
 *
 ...
 *
 * Extraction technique (identical to .com, and why it "looks like a real user"):
 * the in-page bridge PASSIVELY observes the page's own fetch/XHR responses and
 * reads __NEXT_DATA__ — it does not synthesize its own API calls. So every
 * PurchaseHistoryV3 request is a genuine, correctly-headered request the
 * Walmart.ca client made itself; pagination advances by clicking the real
 * "next" button (which the client translates into the cursor request), pacing
 * naturally to the UI and covering every order type the .com adapter covers.
 */
const WalmartCaProvider = (() => {
  "use strict";
  ...
  function isOrdersListUrl(url) {
    return /^https:\/\/www\.walmart\.ca\/orders\/?($|\?)/.test(String(url || ""));
  }
...
function extractItemsFromNextData(orderNode) {
  ...
  const pushItem = (item, groupStatus = '') => {
    ...
    const productLink = canonicalUrl
      ...
      : usItemIdForLink
        ? `https://www.walmart.ca/ip/${usItemIdForLink}`
        : 'N/A';
...
async function checkForNextPage() {
  ...
  return {
    id: 'WALMART_CA',
    label: 'Walmart.ca',
    flag: 'provider.walmart_ca',
    defaultEnabled: false,
    hostPermissions: ['https://www.walmart.ca/*'],
    contentMatches: ['https://www.walmart.ca/orders*'],
    ordersListUrl: 'https://www.walmart.ca/orders',
    locale: 'en-CA',
    currency: 'CAD',
    SELECTORS,
    isOrdersListUrl,
...
if (typeof ProviderRegistry !== 'undefined' && ProviderRegistry.register) {
  ProviderRegistry.register(WalmartCaProvider);
}
